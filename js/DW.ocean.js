...
/*========================================
Calm Ocean Waves
https://codepen.io/jackrugile/pen/bgbdBa
========================================*/
...
DW.ocean.PI = Math.PI;
DW.ocean.TAU = DW.ocean.PI * 2;

DW.ocean.rand = function( min, max ) {
	if ( !max ) {
		var max = min;
		min = 0;
	}
...
DW.ocean.init = function() {
	DW.ocean.c = document.getElementById( 'dw-ocean' );
	DW.ocean.ctx = DW.ocean.c.getContext( '2d' );
	DW.ocean.simplex = new SimplexNoise();
	DW.ocean.events();
	DW.ocean.reset();
	DW.ocean.loop();
};
...
DW.ocean.reset = function() {
	DW.ocean.w = window.innerWidth;
	DW.ocean.h =  window.innerHeight * 2;
	DW.ocean.cx = DW.ocean.w / 2;
	DW.ocean.cy = DW.ocean.h / 2;
	DW.ocean.c.width = DW.ocean.w;
	DW.ocean.c.height = DW.ocean.h;
	
	DW.ocean.count = Math.floor( DW.ocean.w / 50 );
	DW.ocean.xoff = 0;
	DW.ocean.xinc = 0.05;	
	DW.ocean.yoff = 0;
	DW.ocean.yinc = 0.003;
	DW.ocean.goff = 0;
	DW.ocean.ginc = 0.003;
	DW.ocean.y = DW.ocean.h * 0.46;
	DW.ocean.length = DW.ocean.w + 10;
	DW.ocean.amp = 40;
};
...
DW.ocean.events = function() {
	window.addEventListener( 'resize', DW.ocean.reset.bind( this ) );	
};
...
DW.ocean.wave = function(offset) {
	DW.ocean.ctx.beginPath();
	let sway = DW.ocean.simplex.noise2D( DW.ocean.goff, 0 ) * DW.ocean.amp;
	for( let i = 0; i <= DW.ocean.count; i++ ) {
		DW.ocean.xoff += DW.ocean.xinc;
		let x = DW.ocean.cx - DW.ocean.length / 2 + ( DW.ocean.length / DW.ocean.count ) * i;
		let y = DW.ocean.y + DW.ocean.simplex.noise2D( DW.ocean.xoff, DW.ocean.yoff ) * DW.ocean.amp + sway + offset ;
		DW.ocean.ctx[ i === 0 ? 'moveTo' : 'lineTo' ]( x, y );
	}
	DW.ocean.ctx.lineTo( DW.ocean.w, DW.ocean.h);
	DW.ocean.ctx.lineTo( 0, DW.ocean.h);
	DW.ocean.ctx.closePath();

	DW.ocean.ctx.fillStyle = 'rgb('+(5+Math.floor(offset/10))+','+Math.floor(10+offset/6)+','+Math.floor(33+offset/3)+')';
	DW.ocean.ctx.fill();
};
...
DW.ocean.loop = function() {
	requestAnimationFrame( DW.ocean.loop );
	DW.ocean.ctx.clearRect( 0, 0, DW.ocean.w, DW.ocean.h );
	DW.ocean.xoff = 0;
	DW.ocean.wave(-13);
	DW.ocean.wave(0);
	DW.ocean.wave(1);
	...
	DW.ocean.wave(610);
	DW.ocean.wave(987);
	DW.ocean.yoff += DW.ocean.yinc;
	DW.ocean.goff += DW.ocean.ginc;
};
